...
export function getPrismaClient() {
  if (!prismaClient) {
    ...
    if (connectionString.includes('supabase.com') || process.env.DATABASE_SSL_REQUIRED === 'true') {
      // Always use SSL bypass in development to prevent crashes
      // In production, we can use proper SSL validation
      poolConfig.ssl = { rejectUnauthorized: false };
      process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
      console.log('⚠️ Using SSL bypass for Supabase connection');
    }
