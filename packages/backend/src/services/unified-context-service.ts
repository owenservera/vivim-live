...
export class UnifiedContextService {
  ...
  async generateContextForChat(
    ...
    // Use virtualUserId from options if provided, otherwise try to derive from conversationId
    let virtualUserId = options.virtualUserId;

    // Try new dynamic context assembler first
    if (this.config.enableNewContextEngine && this.dynamicAssembler) {
      try {
        // If virtualUserId not provided in options, try to get from conversation
        if (!virtualUserId) {
          virtualUserId = await this.getVirtualUserIdForConversation(conversationId);
        }

        if (!virtualUserId) {
          throw new Error('Could not determine virtual user for conversation');
        }

        // Fetch user settings
        const userSettings = await this.getUserContextSettings(virtualUserId);
   ...
   */
  async warmupBundles(virtualUserId: string, presence: any): Promise<void> {
    if (!this.config.enableNewContextEngine || !this.dynamicAssembler) {
      return;
    }
    ...
    try {
      const result = await this.dynamicAssembler.assemble({
        ...
        settings: await this.getUserContextSettings(virtualUserId),
      });
   ...
   */
  async invalidateBundles(virtualUserId: string, eventType: string, relatedIds: string[]): Promise<void> {
  ...
  private async getVirtualUserIdForConversation(conversationId: string): Promise<string | null> {
    const conv = await prisma.conversation.findUnique({
      where: { id: conversationId },
      select: { virtualUserId: true },
    });
    return conv?.virtualUserId || null;
  }

  private async getUserContextSettings(virtualUserId: string): Promise<any> {
  ...
  async healthCheck(): Promise<{
    ...
    const virtualUserId = 'system'; // System-wide check

    const [topicCount, entityCount, bundleCount] = await Promise.all([
      prisma.topicProfile.count({ where: { virtualUserId } }),
      prisma.entityProfile.count({ where: { virtualUserId } }),
      prisma.contextBundle.count({ where: { virtualUserId } }),
    ]);
