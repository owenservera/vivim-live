...
export class HybridRetrievalService {
  ...
  async retrieve(
    ...
    // Generate cache key
    const cacheKey = this.generateCacheKey(virtualUserId, message, topicSlugs);

    // Check cache first
    const cached = this.getCached(cacheKey);
    if (cached) {
      logger.debug({ cacheKey, virtualUserId }, 'JIT cache hit');
      return cached;
    }

    logger.debug({ cacheKey, virtualUserId }, 'JIT cache miss, performing retrieval');

    const keywords = this.extractKeywords(message);

    const [semanticAcus, keywordAcus, semanticMemories, keywordMemories] = await Promise.all([
      this.semanticSearchACUs(virtualUserId, embedding, topicSlugs),
      this.keywordSearchACUs(virtualUserId, keywords, topicSlugs),
      this.semanticSearchMemories(virtualUserId, embedding),
      this.keywordSearchMemories(virtualUserId, keywords),
    ]);
  ...
  private generateCacheKey(virtualUserId: string, message: string, topicSlugs: string[]): string {
    ...
    const topicsHash = crypto
      ...
      .digest('hex')
      .substring(0, 8);
    return `jit:${virtualUserId}:${messageHash}:${topicsHash}`;
  }
   ...
   */
  clearCacheForUser(virtualUserId: string): void {
    const prefix = `jit:${virtualUserId}:`;
    for (const key of this.jitCache.keys()) {
      if (key.startsWith(prefix)) {
        this.jitCache.delete(key);
      }
    }
    logger.debug({ virtualUserId }, 'JIT cache cleared for user');
  }
  ...
  private async semanticSearchACUs(
    ...
    try {
      ...
      return this.fallbackSemanticSearch(virtualUserId, topicSlugs);
    }
  }
  ...
  private async keywordSearchMemories(virtualUserId: string, keywords: string[]): Promise<SearchResult[]> {
    if (keywords.length === 0) {
      return [];
    }
