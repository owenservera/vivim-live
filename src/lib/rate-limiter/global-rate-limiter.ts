...
export class GlobalRateLimiter extends EventEmitter {
   ...
   * @param options.priority - Request priority (default: NORMAL)
   * @param options.timeoutMs - Custom timeout for this request
  ...
  async submit<T>(
    ...
    return new Promise((resolve, reject) => {
      ...
      // CRITICAL requests go to front of queue if nothing is processing
   ...
   * @param cancelInFlight - Also cancel the currently processing request
   */
  ...
  /**
   * Pause queue processing (requests still queue but don't process)
   */
  pause(): void {
    this.isPaused = true;
  ...
  /**
   * Check if limiter is healthy (queue not near capacity)
   */
  isHealthy(): boolean {
  ...
  /**
   * Destroy the limiter and clear all requests
   */
  destroy(): void {
    this.clear(true);
