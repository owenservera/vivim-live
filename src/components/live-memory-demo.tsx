...
export function LiveMemoryDemo() {
  const t = useTranslations('demos.liveMemory');
  const tc = useTranslations('demos.liveMemory.component');
    ...
    <div className="grid lg:grid-cols-2 gap-6 rounded-2xl overflow-hidden border border-white/10 bg-slate-900/50">
      <div className="flex flex-col h-96">
        <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-slate-800/40">
          ...
          <span className="text-xs text-slate-500 ml-2">{tc('demoChat')}</span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          {isTyping && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 px-3 py-3">
              {[0, 1, 2].map(i => (
        ...
        <div className="border-t border-white/5 p-3">
          <div className="flex gap-2">
            <input
              ...
              placeholder={flowIndex < DEMO_FLOWS.length ? DEMO_FLOWS[flowIndex].userInput : tc('memoryFull')}
        ...
        <div className="flex flex-col h-96 border-l border-white/5">
          <div className="flex items-center gap-2 px-4 py-3 border-b border-white/5 bg-slate-800/40">
            ...
            <span className="text-xs text-slate-500">{tc('liveMemoryExtraction')}</span>
            <span className="ml-auto text-xs text-violet-400 font-mono">{memories.length} {tc('acUs')}</span>
          </div>
              ...
              >
                <div className="flex items-center justify-between gap-2 mb-1">
                  <span className="font-mono font-bold tracking-wide">{mem.type}</span>
                    <span className="opacity-60">{Math.round(mem.confidence * 100)}% {tc('confidence')}</span>
                </div>
